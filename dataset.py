...
class FilmDustSeqDataset(data.Dataset):
    def __init__(self, rootdir):
        self.rootdir = rootdir
        listdir = os.listdir(rootdir)
        alphapaths = list(filter(lambda n: n.endswith("_alpha.png"), listdir))
    ...
    def __getitem__(self, index):
        ...
        prev = img.crop((0, 0, 256, 256))
        cur = img.crop((256, 0, 256 + 256, 256))
        next = img.crop((512, 0, 512 + 256, 256))

        tprev = self.totensor(prev)
        tcur = self.totensor(cur)
        tnext = self.totensor(next)
        timg = torch.cat([tcur, tprev, tnext])
